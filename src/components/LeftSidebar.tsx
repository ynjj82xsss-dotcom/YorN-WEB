...
import { Plus, MessageSquare, Settings, User, X, LogIn, LogOut, Edit2, Trash2 } from 'lucide-react';
...
export default function LeftSidebar({ sessions, currentSessionId, onSelectSession, onDeleteSession, onRenameSession, onNewChat, isOpen, onClose, onOpenProfile, user, onLogin, onLogout }: LeftSidebarProps) {
      ...
      >
        <div className="p-6 flex flex-col h-full w-[280px]">
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-transparent flex items-center justify-center border border-[#333] shadow-[0_0_15px_rgba(255,255,255,0.03)] relative overflow-hidden bg-[#0A0A0A]">
                ...
                <svg viewBox="0 0 100 100" className="w-5 h-5 relative z-10" fill="none" stroke="#E0E0E0" strokeWidth="6" strokeLinejoin="miter">
                  {/* Stylized Y */}
              ...
              <span className="font-medium text-[#A0A0A0] tracking-wide">YorN AI</span>
            </div>
          ...
          <div className="mt-8 flex-1 overflow-y-auto scrollbar-hide">
             <div className="text-[10px] uppercase tracking-[0.2em] text-[#444] font-semibold mb-4 px-2">История</div>
             <div className="flex flex-col gap-1">
               {sessions.map((item) => (
                 <div key={item.id} className="relative animate-[fadeIn_0.15s_ease-out]">
                   <button 
                      onClick={() => onSelectSession(item.id)}
                      onContextMenu={(e) => handleContextMenu(e, item.id)}
                      className={`group flex items-center gap-3 w-full py-2.5 px-3 rounded-lg text-left transition-all duration-200 hover:translate-x-1 relative overflow-hidden ${
                        item.id === currentSessionId 
                          ...
                          : 'hover:bg-[#121212]/50 border-l-2 border-transparent pl-2'
                      }`}
                   >
                     <MessageSquare 
                       size={13} 
                       className={`shrink-0 transition-all duration-300 ${
                         item.id === currentSessionId ? 'text-white scale-110' : 'text-[#444] group-hover:text-[#888]'
                       }`} 
                     />
                     <div className="flex flex-col overflow-hidden max-w-full flex-1">
                        {editingSessionId === item.id ? (
                          <form onSubmit={(e) => handleRenameSubmit(e, item.id)} onClick={(e) => e.stopPropagation()}>
                            <input 
                               type="text" 
                               value={editingTitle}
                          ...
                          <span className={`text-xs transition-all duration-300 truncate ${item.id === currentSessionId ? 'text-[#E0E0E0]' : 'text-[#666] group-hover:text-[#AAA]'}`}>
                            {item.title}
                          </span>
                        )}
                     </div>
                   ...
                   {menuSessionId === item.id && (
                     ...
                     >
                       ...
                       <button 
                         onClick={() => { onDeleteSession(item.id); setMenuSessionId(null); }} 
          ...
          <div className="mt-6 pt-6 border-t border-[#1A1A1A] flex flex-col gap-2">
            {user ? (
               <div className="flex flex-col gap-2">
                  <button onClick={onOpenProfile} className="flex items-center gap-3 w-full py-3 px-2 rounded-lg text-left hover:bg-[#151515] transition-all">
                    ...
                    <div className="flex flex-col flex-1 overflow-hidden">
                      <span className="text-[11px] font-medium text-[#E0E0E0] truncate">{user.displayName || 'Пользователь'}</span>
                      <span className="text-[9px] text-[#555] truncate">{user.email}</span>
                    </div>
                  </button>
                  {confirmLogout ? (
                     <div className="bg-[#0B0B0B] border border-red-950/30 p-3 rounded-lg flex flex-col gap-2.5 shadow-inner">
                        <span className="text-[10px] text-[#888] text-center font-medium">Вы точно хотите выйти?</span>
                     ...
                     <button onClick={() => setConfirmLogout(true)} className="flex items-center gap-3 w-full py-2 px-3 rounded-lg text-left hover:bg-[#151515] transition-all text-[#888] hover:text-[#CCC]">
                       <LogOut size={14} />
               ...
               <button onClick={onLogin} className="flex items-center justify-center gap-2 w-full py-3 px-2 rounded-lg bg-white text-black hover:bg-gray-100 transition-all font-medium">
                  <LogIn size={16} />
